## (기존) network → iam → db → ssm → lb → app → web 순서로 생성
## (수정1) network → kms → iam → db → ssm → bastion(선택) → lb → lambda → ecs_cluster → ecs → web 순서로 생성
## (수정2) network → common → kms → iam → db → ssm → lb → bastion → lambda → ecs → Web
